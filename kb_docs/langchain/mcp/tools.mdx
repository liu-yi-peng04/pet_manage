...
When a tool returns structured content, the adapter attaches it to the @[`ToolMessage`] as an artifact rather than folding it into the model-visible text. Run the agent, then read the `artifact` off the @[`ToolMessage`]s in the result:
